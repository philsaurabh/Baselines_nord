...
current_delta = 0.3  # Initial threshold for likelihood ratio test
delta_increment = 0.1
confidence_threshold = 0.1  # For probabilistic correction
...
for epoch in range(args.epoch):
    ...
    for x, target in train_loader:
        ...
        # Perform label correction using teacher predictions
        target_cpu = target.cpu().numpy()
        if confidence_threshold >= 0.1:
            # Apply likelihood ratio test correction
            target_cpu, current_delta = lrt_correction(
                y_tilde=target_cpu,
                f_x=teacher_probs,
            ...
            # Apply probabilistic label correction
            target_cpu, current_delta = prob_correction(
                y_noise=target_cpu,
                f_x=teacher_probs,
                current_delta=current_delta,
                delta_increment=delta_increment,
                thd=confidence_threshold
            )

        # Convert corrected labels back to tensor
        corrected_targets = torch.tensor(target_cpu).long().cuda()

        # Forward pass through student model
        output, s_feat, _ = s_model(x, bb_grad=True)
        log_nor_output = F.log_softmax(output[nor_index] / args.kd_T, dim=1)

        # Compute refined logits using self-guided soft logits
        t_similarity = F.cosine_similarity(
            t_feat.unsqueeze(1), t_feat.unsqueeze(0), dim=2
        )
        teacher_probs_nor = teacher_probs[:batch]
        t_similarity_nor = t_similarity[:batch, :batch]
        ...
        # Compute losses
        loss1 = F.cross_entropy(output[nor_index], corrected_targets)  # Cross-Entropy Loss
        loss2 = F.kl_div(log_nor_output, refined_probs, reduction="batchmean") * args.kd_T * args.kd_T  # Knowledge Distillation Loss

        # Adversarial Training
        x_adv = x.clone().detach().requires_grad_(True)
        output_adv, _, _ = s_model(x_adv)
        loss_adv = F.cross_entropy(output_adv[nor_index], corrected_targets)
        adv_grad = torch.autograd.grad(loss_adv, x_adv, retain_graph=False, create_graph=False)[0]
        x_adv = torch.clamp(x_adv + 1e-7 * adv_grad.sign(), 0, 1)
        output_adv, _, _ = s_model(x_adv)
        log_adv_output = F.log_softmax(output_adv[nor_index] / args.kd_T, dim=1)
        loss3 = F.kl_div(log_adv_output, refined_probs, reduction="batchmean") * args.kd_T * args.kd_T  # Adversarial Loss

        # Combine losses
        loss = args.ce_weight * loss1 + args.kd_weight * loss2 + 0.0 * loss3
        loss.backward()
        optimizer.step()

        # Update metrics
        cls_batch_acc = accuracy(output[nor_index], corrected_targets, topk=(1,))[0]
