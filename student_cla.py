...
for epoch in range(args.epoch):
    ...
    for x, target in train_loader:   
        ...
        # Forward pass through teacher model (no gradient required)
        with torch.no_grad():
            teacher_output, t_feat, _ = t_model(x)
            teacher_probs = F.softmax(teacher_output[nor_index] / args.kd_T, dim=1)

        # Confident Learning: Calculate confidence scores
        confidence_scores, predictions = torch.max(teacher_probs, dim=1)
        
        # Identify noisy samples (low-confidence ones)
        confidence_threshold = 0.85# e.g., 0.7args.confidence_threshold  
        clean_indices = confidence_scores >= confidence_threshold
        
        # Filtering clean data
        x_clean = x[nor_index][clean_indices]
        target_clean = target[clean_indices]

        if len(x_clean) == 0:  # Skip batch if all samples are noisy
            continue

        # Re-compute outputs for clean data
        output_clean, s_feat_clean, _ = s_model(x_clean)
        log_clean_output = F.log_softmax(output_clean / args.kd_T, dim=1)
        with torch.no_grad():
            teacher_probs_clean = F.softmax(teacher_output[nor_index][clean_indices] / args.kd_T, dim=1)

        # Loss computation
        # Cross-Entropy Loss (Supervision from true labels)
        loss1 = F.cross_entropy(output_clean, target_clean)

        # KL Divergence Loss (Soft label supervision from teacher)
        loss2 = F.kl_div(log_clean_output, teacher_probs_clean, reduction='batchmean') * args.kd_T * args.kd_T
        ...
        # Update metrics
        #loss1_record.update(loss1.item(), len(target_clean))
        #loss2_record.update(loss2.item(), len(target_clean))

   # print(f"Epoch {epoch+1}/{args.epoch}, Loss1: {loss1_record.avg:.4f}, Loss2: {loss2_record.avg:.4f}")
